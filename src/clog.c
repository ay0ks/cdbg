...
#define __clog_clamp($1, $2, $3)                                               \
  ((((typeof($2))($2)) < ((typeof($1))($1)))                                   \
     ? ((typeof($1))($1))                                                      \
     : (((typeof($2))($2)) > ((typeof($3))($3)) ? ((typeof($3))($3))           \
                                                : ((typeof($2))($2))))
...
)
{
  ...
  char *l_message = nullptr;
  char *l_message_2 = nullptr;
  if(a_argc > 0) { l_message = va_arg(l_args, char *); }
  if(a_argc > 1)
  {
    va_list l_args_2;
  ...
  if(l_message != nullptr) { fprintf(stderr, "\n  reason: %s", l_message_2); }
  fprintf(stderr, "\n");
  if(a_abort) { clog_abort(); }
}
  ...
  for(uint64_t l_u = 0; l_u < l_size_total; l_u += 16)
  {
    ...
    if(a_lookaround.m_lookbehind > 0 && l_u < a_lookaround.m_lookbehind)
    {
      fprintf(
        stderr,
        "%06lx",
        __clog_clamp(0, a_lookaround.m_lookbehind - l_u, a_lookaround.m_lookbehind)
      );
    }
            ...
            && l_u <= a_size + a_lookaround.m_lookbehind + a_lookaround.m_lookahead)
    {
      fprintf(
        stderr,
        "%06lx",
        __clog_clamp(0, l_u - a_lookaround.m_lookbehind, a_size + a_lookaround.m_lookahead)
      );
    }
    else
    {
    ...
    for(uint8_t l_i = 0; l_i < 16; l_i += 1)
    {
      ...
      else
      {
        ...
        if(l_tty) { fprintf(stderr, "\x1b[0m"); }
      }
    }
    fprintf(stderr, " ");
    for(uint8_t l_i = 0; l_i < 16; l_i += 1)
